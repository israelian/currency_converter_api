...
@app.get("/convert/")
async def convert_currency(source: str, target: str, amount: float, db: AsyncSession = Depends(get_async_session)):
    async with db as session:
        async with session.begin():
            query = select(models.Currency).where(models.Currency.code.in_([source, target]))
            ...
            source_rate = next((c.rate for c in currencies if c.code == source), None)
            target_rate = next((c.rate for c in currencies if c.code == target), None)
            last_updated_time = next((c.last_updated for c in currencies), None)
            if source_rate is None or target_rate is None:
                raise HTTPException(status_code=404, detail="Currency conversion rate not found.")

            converted_amount = (amount / source_rate) * target_rate
            return {"converted_amount": converted_amount, "last_updated": last_updated_time}
    ...
    async with db as session:
        async with session.begin():
            ...
            if last_update is None:
                raise HTTPException(status_code=404, detail="No currency rates found.")
