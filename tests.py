...
async def clear_database():
    async with get_async_session() as session:
        await session.execute('TRUNCATE TABLE currencies RESTART IDENTITY CASCADE')
...
def mock_fetch_exchange_rates(monkeypatch):
    ...
    monkeypatch.setattr("app.utils.fetch_exchange_rates", AsyncMock(side_effect=mock))
...
async def test_convert_currency_success(client, override_get_async_session):
    async with get_async_session() as session:
        await session.execute(
            "INSERT INTO currencies (code, rate) VALUES ('USD', 1.0), ('EUR', 0.9)"
        )
        await session.commit()
...
async def test_get_last_update_success(client, override_get_async_session):
    async with get_async_session() as session:
        await session.execute(
            "INSERT INTO currencies (code, rate, last_updated) VALUES ('USD', 1.0, NOW())"
        )
        await session.commit()
